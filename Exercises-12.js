...
nama pemblei
barang yg ingin beli
jumlah barang yg dibeli

jika jumlah stock ditoko tidak sesuai dengan jumlah yg mau dibeli maka batal membeli
*/
    ...
    if (shoppers.length == 0) {
        ...
        for (var i = 0; i < listBarang.length; i++) {
            ...
            for (var j = 0; j < shoppers.length; j++) {
                // console.log(listBarang[i], shoppers[j]);
                if (listBarang[i][0] == shoppers[j].product && gudang.leftOver - shoppers[j].amount >= 0) {
                    gudang.shoppers.push(shoppers[j].name);
                    gudang.leftOver -= shoppers[j].amount;
                    gudang.totalProfit += shoppers[j].amount * listBarang[i][1]
                }
            }
            result.push(gudang)
        }
